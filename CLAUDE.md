...
- **`MboxViewer`** — UI + the **index + lazy-load** model. It does NOT keep parsed emails in memory. `displayEmail(email)` renders `bodyHtml` in a **sandboxed `<iframe>`** (markup/CSS render; scripts, forms and top-navigation blocked), falls back to plain text via `textContent`, and lists attachments with Download buttons (`attachmentToBlob` → object URL). `parseMboxFile` is retained only for the Node tests.
...
- **HTML bodies render in a sandboxed iframe, not via `innerHTML`.** Email HTML is untrusted — it goes into `<iframe sandbox="allow-popups allow-popups-to-escape-sandbox allow-same-origin">` (scripts, forms and top-navigation stay blocked). `allow-same-origin` is set only so the iframe can be sized to its rendered content (`renderEmail` reads `contentDocument…scrollHeight` on load) — with no `allow-scripts` and the CSP, the inert email HTML gains nothing from the shared origin. User-clicked links open in a new tab — `buildFrameDocument` rewrites `<a>` to `target="_blank" rel="noopener noreferrer"`. Never switch it to `innerHTML`. Header fields and attachment names still go through `escapeHtml`; keep that. Inline `cid:` images are rewritten to embedded `data:` URLs before rendering (`inlineCidImages`, matching on each part's `contentId`); the iframe also carries a CSP (`default-src 'none'; img-src data:; …`) that blocks every remote subresource, so external (http) images and tracking pixels never load.
